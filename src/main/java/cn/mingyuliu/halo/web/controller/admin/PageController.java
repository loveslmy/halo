...
public class PageController {
    ...
    public JsonResult pushPage(@ModelAttribute Post post, HttpSession session) {
        ...
        try {
            ...
            if (post.getId() > 0) {
                post.setVisitCount(postService.findByPostId(post.getId()).get().getVisitCount());
                post.setPostDate(postService.findByPostId(post.getId()).get().getPostDate());
                post.setUpdDate(currentDate);
                msg = "更新成功";
            } else {
